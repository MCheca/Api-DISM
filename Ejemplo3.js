...
app.get('/observacion', function (req, resp) {
    connection.query('select * from observacion', function (err, rows) {
        if (err) {
            ...
            resp.status(500);
            resp.send({
                message: "Error al obtener observacion"
            });
        } else {
...
app.get('/introducirDatos', function (req, resp) {
                ...
                if (entry.num_hab > 50000) {
                    ...
                    connection.query(sql, function (err, rows) {
                        if (err) {
                            ...
                            resp.status(500);
                            resp.send({
                                message: "Error al insertar municipios"
                            });
                        } else {
    ...
    request('http://vps481071.ovh.net:8080/datosEstaciones', {
        json: true
    }, (err, res, body) => {
        if (err) {
            ...
            datos.forEach(function (entry) {
                ...
                connection.query(sql, function (err, rows) {
                    if (err) {
                        ...
                        resp.status(500);
                        resp.send({
                            message: "Error al insertar estaciones"
                        });
                    } else {
    ...
    request('http://vps481071.ovh.net:8080/datosObservacion', {
        json: true
    }, (err, res, body) => {
        if (err) {
            return console.log(err);
            ...
            datos.forEach(function (entry) {
                ...
                connection.query(sql, function (err, rows) {
                    if (err) {
                        ...
                        resp.status(500);
                        resp.send({
                            message: "Error al insertaciar observacion"
                        });
                    } else {
